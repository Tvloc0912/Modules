...
async function makeImage({ one, two }) {
    ...
    let batgiam_img = await jimp.read(__root + "/tromcho.png");
    ...
    batgiam_img.resize(500, 500).composite(circleOne.resize(50, 50), 234, 50).composite(circleTwo.resize(90, 90), 50, 250);
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("Vui lòng tag 1 người", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body:"hehe " + tag + ' M coi chừng t xích m lại nhé sủa cl! 😈😈',
            mentions: [{
          tag: tag,
          id: mention
        }],
