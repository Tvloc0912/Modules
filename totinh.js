...
async function makeImage({ one, two }) {
   ...
   totinh_img.resize(500, 500).composite(circleOne.resize(90, 90), 142, 86).composite(circleTwo.resize(90, 90), 320, 119);
...
module.exports.run = async function ({ event, api, args, client }) {
    ...
    if (!mention) return api.sendMessage("Vui lòng tag 1 người", threadID, messageID);
		...
		return makeImage({ one, two }).then(path => api.sendMessage({ body: "Trong trường hợp Anh bị say đắm bởi vẻ đẹp quyến rũ của em " + tag + '\n (hoặc những vẻ đẹp tương tự của em), anh khẳng định anh không liên hệ bởi bất cứ một cô gái khác nào trong nhóm này, có lẽ trái tim của anh chỉ dành cho em. Anh cũng xin khẳng định anh không hề có thể yêu một cô gái nào khác khi đã yêu em..',
            mentions: [{
          tag: tag,
          id: mention
        }],
